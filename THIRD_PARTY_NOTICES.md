...
This file accompanies the HarnessDesk Legacy Preview asset. The old installer is not the current HarnessDesk candidate and does not contain this root document at its application root.

The section below is preserved from deepseek-ai/deepseek-harness commit 47f943859bef60e4160492346772ded9b24f765a. It covers the upstream notice generator's declared scope. It is not a complete, newly generated inventory of the separate Electron, Chromium, Node, NSIS, electron-updater, and wrapper closure. Those components retain license files inside the legacy package; a future HarnessDesk candidate must generate and verify the exact shipped closure.
...
The complete npm transitive closure, including the Landlock launcher workspace, is recorded with exact pinned versions in [`pnpm-lock.yaml`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/pnpm-lock.yaml) — inspect it with `pnpm licenses list`. The Python closure is recorded separately in [`python/sdk/uv.lock`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/python/sdk/uv.lock).

## Vendored source (`vendor/`)

The Cordis framework and its foundation libraries are source-vendored into the upstream repository rather than consumed from npm, and republished under the `@deepseek-ai` scope. All are MIT-licensed; each directory preserves its upstream `LICENSE` file. Exact upstream commits and local modifications are recorded in [`vendor/README.md`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/vendor/README.md).
...
- `node-pty@1.1.0` — [`patches/node-pty@1.1.0.patch`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/patches/node-pty@1.1.0.patch)
